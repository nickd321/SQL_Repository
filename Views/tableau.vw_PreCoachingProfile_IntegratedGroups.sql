...
CREATE VIEW	[tableau].[vw_PreCoachingProfile_IntegratedGroups]
AS
...
Notes:		This is a near-total rewrite of the previous view used for this application
			...
			Added hierarchical date fields for Tableau to speed up extract refreshing.
			
			11/15/13 Greg & Kevin discussed with Adam and he requested include all appointment types except 
			Health Navigator (id 27) as participating members with a completed session within the period.
****************************************************************************************************/
			...
			(
				...
				FROM
					(
						...
						WHERE
							AppointmentTypeID <> 27 AND			-- all session types but Health Navigator per Adam V.
