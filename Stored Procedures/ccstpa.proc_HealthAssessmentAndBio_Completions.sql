...
-- Description:	PHA and Completions Report for CCS TPA
--
...
--
-- =============================================
CREATE PROCEDURE [ccstpa].[proc_HealthAssessmentAndBio_Completions]
...
SELECT
	...
	CONVERT(VARCHAR(10),ha.AssessmentCompleteDate,101) AS PHACompleteDate
FROM
...
SELECT
	...
	ISNULL(addr.[State],'') AS [State],
	ISNULL(addr.ZipCode,'') AS ZipCode,
	CONVERT(VARCHAR(10),scr.ScreeningDate,101) AS BiometricsScreeningCompleteDate
FROM
	DA_Production.prod.HealthPlanGroup grp WITH (NOLOCK)
JOIN
	DA_Production.prod.Member mem WITH (NOLOCK)
