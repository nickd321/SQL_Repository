...
--
-- Updates:		WilliamPe 20140324
--				Per conversation with Brenda L., the INNER JOINs to 
--				PHA and BIO were removed.  This was done to allow for members to earn the tobacco credit
--				without having to complete PHA and BIO.  However, you still need to complete the pha and bio
--				to earn outcome measure credits.
--
--				WilliamPe 20140417
--				This report is now sourcing from the Healthyroads database directly and not the incentive table
--				on HrlDw. 
--
-- =============================================
CREATE PROCEDURE [truevalue].[proc_Incentives_BusinessSolver]
...
BEGIN
	...
	LEFT JOIN
		...
		AND	(bio.Deleted = 0)
	WHERE
		clnt.ClientIncentivePlanID = 1058
	ORDER BY
		8,9
	...
	FROM
		...
		LEFT JOIN
			...
			FROM
				#Incentive
			WHERE
				Activity = 'Tobacco Use / Non-Use Attestation' AND
				ActivityValue = 1
			GROUP BY
				MemberID
			) tob
